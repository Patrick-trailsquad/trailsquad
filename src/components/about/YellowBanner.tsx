...
const YellowBanner = () => {
  ...
  return <>
        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-10 gap-16 items-center">
            ...
            <div className="lg:col-span-4 flex gap-8 justify-center items-end">
              {videos.map((video, index) => <div key={video.id} className={`relative group cursor-pointer transform transition-all duration-500 hover:-translate-y-2 hover:scale-105 ${index === 0 ? 'mt-8' : 'mb-8'}`} onClick={() => openVideo(video.videoUrl)}>
                  <div className="relative overflow-hidden rounded-2xl shadow-2xl hover:shadow-3xl transition-all duration-500 group-hover:shadow-terra/20">
                    <img src={video.thumbnail} alt="Trail Squad Video" className="w-52 h-80 md:w-60 md:h-96 object-cover transition-transform duration-700" />
      ...
      {selectedVideo && <VideoLightbox videoUrl={selectedVideo} onClose={closeVideo} />}
    </>;
};
