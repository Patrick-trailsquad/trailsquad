...
const LocationMapModal = ({ open, onOpenChange }: LocationMapModalProps) => {
  ...
  // Chianti Region, Tuscany, Italy coordinates
  const chiantiCoordinates: [number, number] = [11.2094, 43.4847];
  ...
  const initializeMap = async () => {
    ...
    try {
      ...
      map.current.on('load', () => {
        ...
        // Add marker for Chianti
        new mapboxgl.Marker({
          color: '#FFDC00',
          scale: 1.2
        })
          .setLngLat(chiantiCoordinates)
          .setPopup(
            new mapboxgl.Popup({ offset: 25 })
              .setHTML('<h3 style="margin: 0; font-weight: bold;">Chianti Region</h3><p style="margin: 4px 0 0 0;">Tuscany, Italy</p>')
          )
          .addTo(map.current);
