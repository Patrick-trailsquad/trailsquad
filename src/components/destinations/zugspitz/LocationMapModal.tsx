...
const LocationMapModal = ({ open, onOpenChange }: LocationMapModalProps) => {
  ...
  // Garmisch-Partenkirchen, Bavaria, Germany coordinates
  const zugspitzCoordinates: [number, number] = [11.0958, 47.4916];
  ...
  const initializeMap = async () => {
    ...
    try {
      ...
      map.current.on('load', () => {
        ...
        // Add marker for Garmisch-Partenkirchen
        new mapboxgl.Marker({
          color: '#FFDC00',
          scale: 1.2
        })
          .setLngLat(zugspitzCoordinates)
          .setPopup(
            new mapboxgl.Popup({ offset: 25 })
              .setHTML('<h3 style="margin: 0; font-weight: bold;">Garmisch-Partenkirchen</h3><p style="margin: 4px 0 0 0;">Bavaria, Germany</p>')
          )
          .addTo(map.current);
