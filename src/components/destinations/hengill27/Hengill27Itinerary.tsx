...
const days = [
  {
    day: 1,
    date: "torsdag 3. juni 2027",
    title: "✈️ Udrejse til Island",
    items: [
      { icon: Plane, text: "Fly fra København til Keflavík" },
      { icon: Bus, text: "Privat transport til Hveragerði" },
      { icon: Coffee, text: "Check-in på hotellet" },
      { icon: UtensilsCrossed, text: "Fælles middag med squaden" },
    ],
  },
  {
    day: 2,
  ...
  {
    day: 4,
    date: "søndag 6. juni 2027",
    title: "✈️ Hjemrejse",
    items: [
      { icon: Coffee, text: "Morgenmad på hotellet" },
      { icon: Bus, text: "Transport tilbage til Keflavík" },
      { icon: Plane, text: "Fly hjem til København" },
    ],
  },
];

interface Hengill27ItineraryProps {
  variant?: "default" | "overlay";
}

const Hengill27Itinerary = ({ variant = "default" }: Hengill27ItineraryProps) => {
    ...
    <Wrapper className={isOverlay ? "w-full" : "w-full py-12 md:py-20"}>
      <div className={isOverlay ? "" : "container mx-auto px-4 md:px-6"}>
        ...
        <p className={`text-center text-lg mb-12 max-w-2xl mx-auto ${isOverlay ? "text-white/70" : "text-charcoal/70"}`}>
          Fra København til Hengill-vulkanen — trailløb, varme kilder og fællesskab
        </p>
...
export default Hengill27Itinerary;
