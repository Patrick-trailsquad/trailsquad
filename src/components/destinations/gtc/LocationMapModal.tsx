...
const LocationMapModal = ({ open, onOpenChange }: LocationMapModalProps) => {
  ...
  // Courmayeur, Italy coordinates
  const courmayeurCoordinates: [number, number] = [7.0031, 45.7976];
  ...
  const initializeMap = async () => {
    ...
    try {
      ...
      map.current.on('load', () => {
        ...
        // Add marker for Courmayeur
        new mapboxgl.Marker({
          color: '#FFDC00',
          scale: 1.2
        })
          .setLngLat(courmayeurCoordinates)
          .setPopup(
            new mapboxgl.Popup({ offset: 25 })
              .setHTML('<h3 style="margin: 0; font-weight: bold;">Courmayeur</h3><p style="margin: 4px 0 0 0;">Italy</p>')
          )
          .addTo(map.current);
