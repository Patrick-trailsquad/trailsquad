...
const LocationMapModal = ({ open, onOpenChange }: LocationMapModalProps) => {
  ...
  // Mount Vesuvius, Naples, Italy coordinates
  const vesuvioCoordinates: [number, number] = [14.4262, 40.8218];
  ...
  const initializeMap = async () => {
    ...
    try {
      ...
      map.current.on('load', () => {
        ...
        // Add marker for Mount Vesuvius
        new mapboxgl.Marker({
          color: '#FFDC00',
          scale: 1.2
        })
          .setLngLat(vesuvioCoordinates)
          .setPopup(
            new mapboxgl.Popup({ offset: 25 })
              .setHTML('<h3 style="margin: 0; font-weight: bold;">Mount Vesuvius</h3><p style="margin: 4px 0 0 0;">Naples, Italy</p>')
          )
          .addTo(map.current);
