...
const Ilulissat27WaitlistForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Fail-safe: gem altid leadet i databasen først
      await supabase.from("quote_requests").insert({
        full_name: trimmedName,
        email: trimmedEmail,
        phone: trimmedPhone,
        destination: DESTINATION_NAME,
        source: "ilulissat27_waitlist",
      });
    } catch (dbError) {
    ...
    try {
      ...
      setIsSuccess(true);
      toast({ title: "Tak!", description: "Du får besked på email, så snart turen åbner for tilmelding." });
    } catch (error) {
...
export default Ilulissat27WaitlistForm;
