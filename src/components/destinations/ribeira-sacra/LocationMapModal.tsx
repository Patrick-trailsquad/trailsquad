...
const LocationMapModal = ({ open, onOpenChange }: LocationMapModalProps) => {
  ...
  // Luintra, Province of Ourense, Spain coordinates
  const ribeiraCoordinates: [number, number] = [-7.2914, 42.3631];
  ...
  const initializeMap = async () => {
    ...
    try {
      ...
      map.current.on('load', () => {
        ...
        // Add marker for Ribeira Sacra
        new mapboxgl.Marker({
          color: '#FFDC00',
          scale: 1.2
        })
          .setLngLat(ribeiraCoordinates)
          .setPopup(
            new mapboxgl.Popup({ offset: 25 })
              .setHTML('<h3 style="margin: 0; font-weight: bold;">Luintra</h3><p style="margin: 4px 0 0 0;">Province of Ourense, Spain</p>')
          )
          .addTo(map.current);
